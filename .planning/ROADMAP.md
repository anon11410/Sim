...
- [x] 03-02-PLAN.md — TRACER: `World`, the fixed nine-phase `PHASES` table and the tick log, one decade of empty ticks from `--config` to a diffable `ticks.csv`, with guard `7f-agents` added in the same commit as `Household` and `Firm`
- [x] 03-03-PLAN.md — The event stream opened by the endowment, the provenance table with its eager header, and `run_meta.json` with the build-time compiler string
- [x] 03-04-PLAN.md — The schema generated from the writers rather than a second derive, committed, and its drift test proved to fire
- [x] 03-05-PLAN.md — The determinism suite: same-seed in-process and cross-process, different-seed, the enforced exclusion, and the process-level halt at tick 0
...
| 3. World, Tick Pipeline and Log Seam | 5/6 | In Progress|  |
