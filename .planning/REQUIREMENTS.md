...
- [ ] **LEDG-06**: Non-negativity is checked every tick across cash, inventory and headcount
- [ ] **LEDG-07**: Zero-sum trade is checked: every sale moves units one way and equal cash the other
- [ ] **LEDG-08**: A liveness invariant asserts transactions-per-tick is greater than zero, closing the "money conserves because nothing trades" degenerate pass
- [ ] **LEDG-09**: On violation the sim halts immediately and prints the tick, the agent and the offending transaction, bisected from a per-tick journal buffer
