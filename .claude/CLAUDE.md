...
| `HashMap` iteration inside `serde` | **Half true, and the false half is the dangerous one.** `serde_json`'s `Value::Object` really is a `BTreeMap` by default, so a `Value` is key-ordered. But a **hashed-map field on a serialised struct** does not go through `Value` at all — it takes the map serialisation path and keeps the map's own iteration order. Measured on this repo: five different orderings in five consecutive runs of one binary. *(Corrected 2026-08-31 in plan 03-03; evidence and method in `src/log.rs`'s module doc.)* | Never put a hashed map on a serialised type. The `clippy.toml` type ban plus `tests/lints.sh` check 4a — which catches a type *alias* the lint cannot see — is what enforces it. Leave `preserve_order` off; it changes `Value`'s backing map and cargo unifies features across the graph, so enabling it is a byte-shape change any future dependency could impose. |
