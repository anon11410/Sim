...
- **Global scope**: `/home/user/Sim/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/user/Sim/.claude/notes/`)
...
2. Glob `/home/user/Sim/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/user/Sim/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/user/Sim/.claude/notes/` — works in any directory
